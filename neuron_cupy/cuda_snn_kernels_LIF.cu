...
__global__ void forward_kernel_fp32(
    ...
    for (int t = 0; t < time_steps; t++) {
        ...
        v -= (v_th[0] * spike); // soft reset
        // v -= (v * spike); // hard reset
...
__global__ void forward_kernel_fp16(
    ...
    for (int t = 0; t < time_steps; t++) {
        ...
        v = __hsub(__hadd(v, x_seq[current_idx]),v/tau);
        H_seq_out[next_idx] = v;
        ...
        v = __hsub(v, __hmul(v_th[0], spike)); //soft reset
        // v = __hsub(v, __hmul(v, spike)); //hard reset
