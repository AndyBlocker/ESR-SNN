...
# Bench: train ANN for 1 epoch, 500 steps; report peak GPU memory and avg time/step.
#
# Usage:
#   ./scripts/bench_vit-base_ANN_train_500step.sh
#   CUDA_VISIBLE_DEVICES=0 DATA_PATH=/path/to/imagenet ./scripts/bench_vit-base_ANN_train_500step.sh
...
OMP_NUM_THREADS="$OMP_NUM_THREADS" CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" \
  python -m torch.distributed.launch --use-env --nproc_per_node=1 --master_port="$MASTER_PORT" \
    ...
    --output_dir "$OUT_DIR" \
    --log_dir "$LOG_DIR" \
    --mode "ANN" --global_pool --act_layer relu \
