...
# Bench: train BiasAllocator (freeze all but BiasAllocator) for 1 epoch, 500 steps.
# Reports peak GPU memory and avg time/step.
#
# Usage:
#   ./scripts/bench_vit-base_BiasAllocator_train_500step.sh
#   CUDA_VISIBLE_DEVICES=0 DATA_PATH=/path/to/imagenet ./scripts/bench_vit-base_BiasAllocator_train_500step.sh
...
OMP_NUM_THREADS="$OMP_NUM_THREADS" CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" \
  python -m torch.distributed.launch --use-env --nproc_per_node=1 --master_port="$MASTER_PORT" \
    ...
    --project_name "T-SNN-Bench-BiasAllocator" \
    --data_path "$DATA_PATH" \
    --output_dir "$OUT_DIR" \
    --log_dir "$LOG_DIR" \
    --neuron_impl torch \
