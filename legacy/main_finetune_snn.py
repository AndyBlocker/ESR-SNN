...
from spike_quan_wrapper import myquan_replace, remove_softmax
from SNNWrapper import SNNWrapper

import models_vit
import wandb

from engine_finetune_snn import train_one_epoch, evaluate, train_one_epoch_distill
...
def main(args):
    ...
    if args.mode.count("QANN") > 0:
        ...
        # manually initialize fc layer
        # trunc_normal_(model.head.weight, std=2e-5)
        model = SNNWrapper(model=model,
                           level=args.level,time_step=args.time_step)
        if args.rank == 0:
            print("======================== SNN model =======================")
