...
from spike_quan_wrapper_ICML import myquan_replace, SNNWrapper, myquan_replace_resnet, SNNWrapper_MS, MyBatchNorm1d, remove_softmax, add_bn_in_mlp, DyHT
...
def get_args_parser():
    ...
    parser.add_argument('--profile_time', action='store_true', default=False,
                        help='profile training time for optimization')
...
# tfwriter = SummaryWriter(log_dir="/home/kang_you/SpikeZIP_transformer_resnet1/output")

def hook_layernorm(name):
    name = name
    step = 0
    mean_list = []
    var_list = []
    def _hook_layernorm(module, inp, out):
        global tfwriter
        nonlocal name, step
        mean = inp[0].mean(dim=-1).detach().cpu()
        var = inp[0].var(dim=-1, unbiased=False).detach().cpu()
        if len(mean.shape) == 2:
            mean_list.append(mean.reshape(-1))
            var_list.append(var.reshape(-1))
        if step == 222 and len(mean_list) > 0:
            # print(mean_list)
            # print(mean_list)
            torch.save(torch.stack(mean_list,dim=0),f"/home/kang_you/SpikeZIP_transformer_resnet1/output/mean_var/{name}_mean.pth")
            torch.save(torch.stack(var_list,dim=0),f"/home/kang_you/SpikeZIP_transformer_resnet1/output/mean_var/{name}_var.pth")
        # tfwriter.add_histogram(tag=f"{name}/mean", values=(inp[0].mean(dim=[0,-1])).detach().cpu(), global_step=step)
        # tfwriter.add_histogram(tag=f"{name}/std", values=(inp[0].var(dim=[0,-1], unbiased=False)).detach().cpu(), global_step=step)
        step = step + 1
        # else:
        #     torch.save()
    return _hook_layernorm
...
def main(args):
    ...
    if args.NormType == "mybatchnorm":
        add_bn_in_mlp(model, normLayer)

    # for layer in model.modules():
    #     if isinstance(layer, nn.Linear) or isinstance(layer, nn.Conv2d):
    #         # 对全连接层或卷积层应用 Xavier 初始化
    #         torch.nn.init.xavier_uniform_(layer.weight)
    #         if layer.bias is not None:  # 如果有偏置，初始化为0
    ...
    if args.mode.count("QANN") > 0:
        if args.model == "resnet50":
            myquan_replace_resnet(model, args.level, args.weight_quantization_bit)
            print(model)
        else:
        ...
        if args.model == "resnet50" or args.model == "resnet18" or args.model == "resnet34":
            myquan_replace_resnet(model, args.level, args.weight_quantization_bit)
        else:
            myquan_replace(model, args.level, args.weight_quantization_bit, is_softmax = not args.remove_softmax)
        if args.ratio > 0.0:
    ...
    if args.eval:
        if args.distributed:
            QANNmodel = torch.nn.parallel.DistributedDataParallel(QANNmodel, device_ids=[args.gpu],find_unused_parameters=True)
        if args.energy_eval:
            ...
            # handles = []
            # for name, module in list(QANNmodel.named_modules()):
            #     if isinstance(module, nn.LayerNorm):
            #         hook = hook_layernorm(name=name)
            #         handles.append(module.register_forward_hook(hook))
            test_stats = evaluate(data_loader_val, QANNmodel, device, None, "QANN", args)
            # for h in handles:
            #     h.remove()            
    ...
    if args.model.count("vit") > 0:
        ...
        # optim_factory.Lamb(param_groups, trust_clip=True, lr=args.lr)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.profile_time:
            ...
            prof.export_chrome_trace(os.path.join(args.output_dir, "trace.json.txt"))
            prof.export_stacks(os.path.join(args.output_dir, "profiler_stacks.txt"), "self_cuda_time_total")
            break
        else:
            train_stats = train_one_epoch(
                model, criterion, data_loader_train,
