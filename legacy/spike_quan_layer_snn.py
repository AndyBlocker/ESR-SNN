...
class Spiking_LayerNorm(nn.Module):
    ...
    def forward(self,input):
        ...
        T = 4
        N = input.shape[0]
        input = input.reshape(torch.Size([int(T),N//int(T)]) + input.shape[1:])        
        for t in range(T):
            self.X = self.X.detach() + input[t]
...
class spiking_softmax(nn.Module):
    ...
    def forward(self, input):
        # print("spiking softmax:",torch.abs(input).mean())
        output_seq = []
        T = 4
        N = input.shape[0]
        input = input.reshape(torch.Size([int(T),N//int(T)]) + input.shape[1:])
        for t in range(T):
            self.X = input[t] + self.X.detach()
...
def multi(x1_t,x2_t,x1_sum_t,x2_sum_t,T):
    # print("multi:",torch.abs(x1_t).mean(),torch.abs(x2_t).mean(),torch.abs(x1_sum_t).mean(),torch.abs(x2_sum_t).mean())
    output_seq = []
    N = x1_t.shape[0]
    x1_t = x1_t.reshape(torch.Size([int(T),N//int(T)]) + x1_t.shape[1:])
    x2_t = x2_t.reshape(torch.Size([int(T),N//int(T)]) + x2_t.shape[1:])
    x1_sum_t = x1_sum_t.reshape(torch.Size([int(T),N//int(T)]) + x1_sum_t.shape[1:])
    x2_sum_t = x2_sum_t.reshape(torch.Size([int(T),N//int(T)]) + x2_sum_t.shape[1:])
    for t in range(T):
        output_seq.append(x1_sum_t[t] @ x2_t[t].transpose(-2, -1) + x1_t[t] @ x2_sum_t[t].transpose(-2, -1) - x1_t[t] @ x2_t[t].transpose(-2, -1))

    return torch.cat(output_seq,dim=0)

def multi1(x1_t,x2_t,x1_sum_t,x2_sum_t,T):
    # print("multi1:",torch.abs(x1_t).mean(),torch.abs(x2_t).mean(),torch.abs(x1_sum_t).mean(),torch.abs(x2_sum_t).mean())
    output_seq = []
    N = x1_t.shape[0]
    x1_t = x1_t.reshape(torch.Size([int(T),N//int(T)]) + x1_t.shape[1:])
    x2_t = x2_t.reshape(torch.Size([int(T),N//int(T)]) + x2_t.shape[1:])
    ...
    x2_sum_t = x2_sum_t.reshape(torch.Size([int(T),N//int(T)]) + x2_sum_t.shape[1:])
    for t in range(T):
        output_seq.append(x1_sum_t[t] @ x2_t[t] + x1_t[t] @ x2_sum_t[t] - x1_t[t] @ x2_t[t])
...
class SAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = multi(q,k,q_acc.float(),(self.k_IF.acc_q*self.k_IF.q_threshold).float(),self.level)
        ...
        if not self.is_softmax:
            x = multi1(attn,v,(acc_attn).float(),(self.v_IF.acc_q*self.v_IF.q_threshold).float(),self.level)
        else:
            x = multi1(attn,v,(self.attn_IF.acc_q*self.attn_IF.q_threshold).float(),(self.v_IF.acc_q*self.v_IF.q_threshold).float(),self.level)
