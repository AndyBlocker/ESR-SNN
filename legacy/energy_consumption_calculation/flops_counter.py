...
import re
from .ops import IFNeuron,ST_BIFNeuron_MS,MyBatchNorm1d, LLConv2d_MS, LLLinear_MS, AttentionMulti, AttentionMulti1, DyT, LLConv2d, LLLinear
from .engine import SNNWrapper_MS,MyQuan, IFNeuron
...
ssa_info = {'depth': 12, 'Nheads': 12, 'embSize': 784, 'patchSize': 16, 'Tsteps': 4}  # small
...
def get_energy_cost(model, ssa_info):
    ...
    Nac = 0
    Nmac = 0
    block_per_sop = [0 for i in range(12)]
    for name, module in model.named_modules():
        ...
        if isinstance(module,nn.Conv2d) or isinstance(module,nn.Linear) or isinstance(module,LLConv2d) or isinstance(module,LLLinear) or isinstance(module,LLConv2d_MS) or isinstance(module,LLLinear_MS) or isinstance(module,DyT) or isinstance(module, nn.AdaptiveAvgPool1d) or isinstance(module,MyBatchNorm1d) or isinstance(module,ST_BIFNeuron_MS) or isinstance(module,AttentionMulti) or isinstance(module,AttentionMulti1): # SpikeZIP: linear in name
            # if 'block' in name:
            ...
            if name.count("block") > 0:
                block_id = name.split(".")[3]
                if name.count("attn.qkv") > 0:
                    block_per_sop[int(block_id)] += float(accumulated_syops_cost[2])
                if name.count("attn.proj_IF") > 0:
                    block_per_sop[int(block_id)] += float(accumulated_syops_cost[2])
                if name.count("mlp.fc1") > 0:
                    block_per_sop[int(block_id)] += float(accumulated_syops_cost[2])
                if name.count("mlp.fc2.0") > 0:
                    block_per_sop[int(block_id)] += float(accumulated_syops_cost[2])
    ...
    # print(isinstance(model.module,SNNWrapper_MS))
    # if isinstance(model.module,SNNWrapper_MS):
    #     print('SSA info: \n', ssa_info)
