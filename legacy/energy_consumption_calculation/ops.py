...
from spike_quan_layer import IFNeuron,QAttention,QuanConv2d,QuanLinear,MyQuan,ST_BIFNeuron_MS, MyBatchNorm1d, LLLinear_MS, LLConv2d_MS, AttentionMulti, AttentionMulti1, DyT, LLConv2d, LLLinear
...
def lllinear_syops_counter_hook(module:LLLinear_MS, input, output):
    input = input[0]  # input is tuple, input[0].shape = torch.Size([4, 64, 384]) [TB, N, C]  # output.shape = torch.Size([4, 64, 384])
    # print(input.shape)
    spike, rate, spkhistc = spike_rate(input)  # 计算了前一层的发放率  # input.unique --> [0,1,2]  spike=False 不把该层作为spike-triggered
    # print("spike, rate",spike, rate)
    ...
    output_last_dim = output.shape[-1]
    # bias_syops = output_last_dim if module.bias is not None else 0
    bias_syops = output_last_dim*batch_size if module.linear.bias is not None else 0 # need to take batch_size into account, as in conv_syops_counter_hook
    module.__syops__[0] += int(np.prod(input.shape) * output_last_dim + bias_syops)
    if spike:
        module.__syops__[1] += int(np.prod(input.shape) * output_last_dim + bias_syops) * rate
    else:
...
def llconv_syops_counter_hook(conv_module:LLConv2d_MS, input, output):
    ...
    kernel_dims = list(conv_module.conv.kernel_size)
    in_channels = conv_module.conv.in_channels
    out_channels = conv_module.conv.out_channels
    groups = conv_module.conv.groups
    padding = conv_module.conv.padding[0]
    stride = conv_module.conv.stride[0]
    # if spike==True:
...
def multihead_window_attention_counter_hook(multihead_attention_module, input, output):
    ...
    multihead_attention_module.__syops__[0] += int(overall_syops)

    if spike1:
        multihead_attention_module.__syops__[1] += int(overall_syops_sparsity)
    else:
        multihead_attention_module.__syops__[2] += int(overall_syops)

    multihead_attention_module.__syops__[3] += (rate1+rate2) * 100 / 2
...
MODULES_MAPPING = {
    ...
    LLConv2d_MS: llconv_syops_counter_hook,
    LLConv2d: llconv_syops_counter_hook,
    ...
    LLLinear: lllinear_syops_counter_hook,
    LLLinear_MS: lllinear_syops_counter_hook,
    ...
    AttentionMulti: multihead_window_attention_counter_hook,
    AttentionMulti1: multihead_window_attention_counter_hook,
