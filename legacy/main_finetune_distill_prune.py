...
from engine_finetune import evaluate, train_one_epoch_distill_prune
...
def main(args):
    ...
    # prepare the prune ratios for each epoch
    if args.ratio == 0.0:
        args.ratio = [0.0 for i in range(args.epochs)]
    else:
        start_ratio = 0.0
        end_ratio = args.ratio
        args.ratio = list(np.linspace(start=start_ratio,stop=end_ratio,num=args.epochs//2))
        for i in range(args.epochs - args.epochs//2):
            args.ratio.append(end_ratio)
    print("the prune ratio for per epoch:",args.ratio)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_distill_prune(
